...
enum AdvanceService {
    ...
    static func recordMutualDebtOffset(
        ...
        let marker = AdvanceSemantics.mutualDebtOffsetMarker(id: offsetGroupID)
    ...
    static func rollbackMutualDebtOffset(
        ...
        try rollbackSpecialRepaymentGroup(
            matching: { AdvanceSemantics.mutualDebtOffsetID(from: $0.note) == offsetGroupID },
            modelContext: modelContext
        )
    }
    ...
    static func recordManualDebtSettlement(
        ...
        let settlementID = UUID()
        let marker = AdvanceSemantics.manualDebtSettlementMarker(id: settlementID)
        let baseNote = note.trimmingCharacters(in: .whitespacesAndNewlines)
    ...
    static func rollbackManualDebtSettlement(
        ...
        try rollbackSpecialRepaymentGroup(
            matching: { AdvanceSemantics.manualDebtSettlementID(from: $0.note) == settlementID },
            modelContext: modelContext
        )
    }
    ...
    static func recordRepayment(
        ...
        direction: AdvanceSemantics.SettlementDirection? = nil,
        autosave: Bool = true,
        modelContext: ModelContext
    ...
    static func updateRepayment(
        ...
        guard !AdvanceSemantics.isMutualDebtOffset(note: repayment.note),
              !AdvanceSemantics.isManualDebtSettlement(note: repayment.note) else {
    ...
    private static func validateSettlementCategory(
        _ category: Category?,
        direction: AdvanceSemantics.SettlementDirection
    ) throws {
    ...
    static func deleteAdvanceCase(
        ...
        return try LedgerMutationService.atomic(modelContext: modelContext, commit: autosave) {
            ...
            var transactionsToDelete: [FinancialTransaction] = []
            ...
            if deleteLinkedTransactions {
                ...
                for groupID in groupIDs {
                    ...
                    transactionsToDelete.append(contentsOf: linkedTransfers)
                }
                ...
                if let expenseID = advanceCase.selfExpenseTransactionID {
                    ...
                    if let expenseTx = try fetchLinkedTransactions(modelContext, descriptor).first {
                        transactionsToDelete.append(expenseTx)
            ...
            let affectedBudgetKeys = transactionsToDelete.compactMap(BudgetHistoryService.affectedKey(for:))
            for transaction in transactionsToDelete { modelContext.delete(transaction) }
